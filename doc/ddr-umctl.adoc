...
`enable_half_bus` or the `enable_quarter_bus` parameter. The effective
bus width must be at least 8 bits, such that `enable_quarter_bus` is
only supported on `sparx5`.
